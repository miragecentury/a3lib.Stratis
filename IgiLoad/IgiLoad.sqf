...
IL_Supported_A3LIB_MidBox = ["Land_Cargo20_white_F"];
IL_Supported_A3LIB_LargeBox = [""];
//TODO
...
IL_Supported_Cargo_Kamaz = IL_Supported_Quadbike + IL_Supported_Supply_Crate + IL_Supported_Veh_Ammo + IL_Supported_Barrel + IL_Supported_Tank + IL_Supported_Rubberboat + IL_Supported_Box_H1 + IL_Supported_Box_H2 + IL_Supported_SDV + IL_Supported_A3LIB_LargeBox + IL_Supported_A3LIB_MidBox;
...
IL_Init_Box =
{
	...
	if (_obj_type in IL_Supported_A3LIB_MidBox) then
	{
		_obj setVariable["slots", 6, true];
		_obj setVariable["zload", 1.9, true];
		_obj setVariable["cargo_offset", 1.6, true];
	...
	{
		{
			...
			if ((typeOf _x) in IL_Supported_A3LIB_MidBox) then
			{
				_turn = true;
			};
			...
			else
			{
				...
				if ((_box_num > _slot_num) && !_done) then
				{
					...
					if (((_obj_type in IL_Supported_Vehicles_HEMTT) || (_obj_type in IL_Supported_Vehicles_KAMAZ)) && (_doors == "B")) then
					{
						...
						if((typeOf _x) in IL_Supported_A3LIB_MidBox)then
						{
							[_v, _x, [0,-5 - _cargo_offset,_zload + 1], [0 + 0.05,_counter -0.75 - _cargo_offset,_zload-0.25], 1, _turn] call IL_Move_Attach;
							hint "yeiueijuoizejdoiej";
							[west,"HQ"] sideChat "yuaoruaoijdoiqsjqsdqsjo";
						}else{
							[_v, _x, [0,-6 - _cargo_offset,_zload], [0,_counter + 0.25 - _cargo_offset,_zload], 1, _turn] call IL_Move_Attach;
						};
						[west,"HQ"] sideChat format["%1",(typeOf _x)];
					};
					if ((_obj_type in IL_Supported_Vehicles_MOHAWK)  && (_doors == "B")) then
					{
			...
			if (_x getVariable "doors" == _doors) then
			{
				...
				if (((_x getVariable "attachedTruck") == _v) && ((_x getVariable "attachedPos") == (_counter)) && (_counter < 0) && !_done) then
				{
					...
					if (((_obj_type in IL_Supported_Vehicles_HEMTT) || (_obj_type in IL_Supported_Vehicles_KAMAZ)) && (_doors == "B")) then
					{
						if((typeOf _x) in IL_Supported_A3LIB_MidBox)then
						{
							[_v, _x, [0,_counter + 0.25 - _cargo_offset -2,_zload], [0,-6 - _cargo_offset -2,_zload], 1, _turn] call IL_Move_Attach;
						}else{
